...
//#ifndef（如果***没被定义，就执行） 起到的效果是防止一个源文件两次包含同一个头文件
...
typedef enum  //定义枚举
{
	...
	//...
} status_t;  //status_t是enum的别名，可以使用status_t来定义枚举类型变量
...
class LIS3DHCore   //定义一个类 
{
public:    //公共成员，外部可以访问
	LIS3DHCore( uint8_t );  //类的构造函数
	LIS3DHCore( uint8_t, uint8_t );  //可以多个，带参数
	~LIS3DHCore() = default;  //析构函数。构造函数和析构函数是一对，构造函数用来创建对象，而析构函数是用来撤销对象。
