#include "KSRobot_Display.h"

KSRobot_Display::KSRobot_Display(uint16_t width_, uint16_t height_)
{
  width = width_ - 1;
  height = height_ - 1;
  rawWidth = width - 1;
  rawHeight = height - 1;
  textSize = 1;
  pfCharacterFont = KSRobot_Character_getCharacter;
  lineWidth = 1;
}

size_t KSRobot_Display::write(uint8_t ch)
{
  _DEBUG_PRINT("\n  print char");
...
size_t KSRobot_Display::write(const uint8_t* pCh, size_t size)
{
  _DEBUG_PRINT("\n  print string");
...
int16_t KSRobot_Display::drawText(int16_t* pX, int16_t* pY, const char* ch)
{
...
void KSRobot_Display::drawVLine(int16_t x, int16_t y, int16_t height_, uint16_t color)
{
...
void KSRobot_Display::drawHLine(int16_t x, int16_t y, int16_t width_, uint16_t color)
{
...
void KSRobot_Display::drawRect(int16_t x, int16_t y, int16_t width, int16_t height, 
                                       uint16_t color)
{
...
void KSRobot_Display::fillRect(int16_t x, int16_t y, int16_t width, int16_t height, 
                                           uint16_t color)
{
  int8_t        directionX = 1;
...
void KSRobot_Display::drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, 
                                       uint16_t color)
{
...
void KSRobot_Display::drawCircle(int16_t x0, int16_t y0, int16_t r, uint16_t color)
{
  r = abs(r);
...
void KSRobot_Display::fillCircle(int16_t x0, int16_t y0, int16_t r, uint16_t color)
{ 
  r = abs(r);
...
void KSRobot_Display::drawTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, 
...
void KSRobot_Display::fillTriangle(int16_t x, int16_t y, int16_t x1, int16_t y1, 
                                              int16_t x2, int16_t y2, uint16_t color)
{
...
void KSRobot_Display::drawRoundRect(int16_t x, int16_t y, int16_t w,
...
void KSRobot_Display::fillRoundRect(int16_t x, int16_t y, int16_t w,
                                        int16_t h, int16_t r, uint16_t color)
{
  r = abs(r);
  if(w > 0) {
...
void KSRobot_Display::circleHelper(int16_t x0, int16_t y0,
...
void KSRobot_Display::fillCircleHelper(int16_t x0, int16_t y0, int16_t r,
...
int32_t KSRobot_Display::readN(Stream * s, uint8_t* pBuf, int32_t length)
{
  int32_t l=length;
  if(pBuf == 0) {
...
int16_t KSRobot_Display::drawBmp(Stream *s, int16_t x, int16_t y)
{
...
void KSRobot_Display::drawBmp(uint8_t* pData, int16_t x, int16_t y, uint16_t w, uint16_t h,
...
int16_t KSRobot_Display::setRotaion(eROTATION eRotation_)
{
  switch(eRotation) {
...
eROTATION KSRobot_Display::getRotation(void)
{
  return eRotation;
}

void KSRobot_Display::setTextColor(uint16_t color)
{
  textColor = color;
}

int16_t KSRobot_Display::getTextColor(void)
{
  return textColor;
}

void KSRobot_Display::setTextBackground(uint16_t color)
{
  textBackground = color;
}

int16_t KSRobot_Display::getTextBackground()
{
  return textBackground;
}

void KSRobot_Display::setTextSize(uint8_t size)
{
  textSize = size;
}

uint8_t KSRobot_Display::getTextSize(void)
{
  return textSize;
}

void KSRobot_Display::setLineWidth(uint16_t w)
{
  lineWidth = w;
}

uint16_t KSRobot_Display::getLineWidth()
{
  return lineWidth;
}

int16_t KSRobot_Display::getWidth(void)
{
  return width;
}

int16_t KSRobot_Display::getHeight(void)
{
  return height;
}

void KSRobot_Display::setDisplayShape(eSHAPE eShape)
{
  switch(eShape) {
...
eSHAPE KSRobot_Display::getDisplayShape(void)
{
  return eShape;
}

int16_t KSRobot_Display::getDisplayRadius(void)
{
...
void KSRobot_Display::setCursor(int16_t x, int16_t y)
{
  if(x > width) {
    printfX = width;
  } else {
...
void KSRobot_Display::getCursor(int16_t* pX, int16_t* pY)
{
  *pX = cursorX;
  *pY = cursorY;
}

void KSRobot_Display::supportChineseFont(void)
{
    pfCharacterFont = GT30L_getCharacter;
}

void KSRobot_Display::setOrign(int16_t x, int16_t y)
{
  if(x > width) {
    cursorX = width;
  } else {
...
void KSRobot_Display::getOrign(int16_t* pX, int16_t* pY)
{
  *pX = printfX; *pY = printfY;
}

void KSRobot_Display::drawBuffer_16(int16_t x, int16_t y, uint16_t* pBuf, uint16_t count)
{

}

void KSRobot_Display::drawBuffer_24(int16_t x, int16_t y, uint8_t* pBuf, uint16_t count)
{
  uint16_t        i = 0;
  for(i = 0; i < (count / 3); i ++)
